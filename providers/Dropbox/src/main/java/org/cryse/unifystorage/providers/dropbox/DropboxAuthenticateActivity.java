...
public class DropboxAuthenticateActivity extends AppCompatActivity {
    ...
    public static boolean checkAppBeforeAuth(Context context, String appKey, boolean alertUser) {
        ...
        if (null == activities || 0 == activities.size()) {
            throw new IllegalStateException("URI scheme in your app's " +
                    ...
                    DropboxAuthenticateActivity.class.getName() + " with the " +
                    "scheme: " + scheme);
            ...
            if (null == resolveInfo || null == resolveInfo.activityInfo
                ...
                throw new IllegalStateException("There must be a " +
                        DropboxAuthenticateActivity.class.getName() + " within your app's package " +
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(intent.hasExtra(DropboxConst.PARCELABLE_NAME_WEB_HOST))
            mWebHost = intent.getStringExtra(DropboxConst.PARCELABLE_NAME_WEB_HOST);
        else
            mWebHost = DEFAULT_WEB_HOST;
        if(intent.hasExtra(DropboxConst.PARCELABLE_NAME_API_TYPE))
            mApiType = intent.getStringExtra(DropboxConst.PARCELABLE_NAME_API_TYPE);
        else
            mApiType = DEFAULT_WEB_HOST;

        mDesiredUid = null;
        mAlreadyAuthedUids = null;
